package org.secuso.privacyfriendlyboardgameclock.backup
...
class BackupRestorer : IBackupRestorer {
    ...
    private fun readDatabase(reader: JsonReader, context: Context) {
        ...
        val actualDatabaseFile = context.getDatabasePath(DbHelper.DB_NAME)

        DatabaseUtil.deleteRoomDatabase(context, DbHelper.DB_NAME)
    ...
    private fun readPreferences(reader: JsonReader, preferences: SharedPreferences.Editor) {
        ...
        while (reader.hasNext()) {
            ...
            when (name) {
                "IsFirstTimeLaunch",
                "showSwipeDialog" -> preferences.putBoolean(name, reader.nextBoolean())
                "gameNumber" -> preferences.putInt(name, reader.nextInt())
